...
- It is recommended to install or perform migration of the OpsHub Integration Manager in the /opt folder or /user/local folder.  
  - For OpsHub Integration Manager migration, if the OpsHub Integration Manager is not installed at the above places, then follow the steps mentioned [here](faqs/general/how-to-move-application-server-from-one-machine-to-other).
